...
import KeyFeaturesModalFurniture from './KeyFeatureModalFurniture';
  ...
  const fetchProduct = async () => {
    ...
    if (true) {
      setExtraDetails({
        ...
        coverMaterial:data?.secondaryCamera,
        ingredients: data?.ingredients || [],
        howToUse: data?.howToUse,
        flavourNote: data?.flavourNote,
        weight: data?.shelfLife,
        suitableFor: data?.type ,
        returnPolicy: data?.returnPolicy ,
        countryOfOrigin: data?.countryOfOrigin,
        WxHxD: data?.seller,
        gpsSupport: data?.sellerAddress,
        battery: data?.sellerFSSAI,
        quickCharging: data?.FSSAILicense,
        customerCareEmail: data?.customerCareEmail,
        warranty: data?.warranty,
        inTheBox: data?.disclaimer,
      });
    }
...
<View style={styles.featuresRow}>
  ...
  {product.seller && product.seller !== 'N/A' && (
    <>
      <CustomText variant="h7">WxHxD</CustomText>
      <CustomText variant="h8" fontFamily={Fonts.SemiBold} style={{ opacity: 0.7 }}>{product.seller}</CustomText>
    </>
  )}

  {product.shelfLife && product.shelfLife !== 'N/A'  && (
    <>
      <CustomText variant="h7" style={{ marginTop: 10 }}>Weight</CustomText>
  ...
  {product.numberOfQuantity && product.numberOfQuantity !== 'N/A' && product.dietType !== 0  && (
    <>
      <CustomText variant="h7" style={{ marginTop: 10 }}>Available Quantity</CustomText>
      <CustomText variant="h8" style={{ opacity: 0.7 }} fontFamily={Fonts.SemiBold}>
...
export default WithCart(FurnitureDetail);   */
