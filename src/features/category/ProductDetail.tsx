...
const ProductDetail = () => {
  ...
  const productId =
    locState?.productId || searchParams.get('productId');

  const categoryId =
    locState?.categoryId || searchParams.get('categoryId');
  ...
  useEffect(() => {
        ...
        if (found) {
          ...
          if (
            ...
            window.history.replaceState(
              {},
              '',
              `/product/${found.slug}?productId=${productId}`
            );
          }
        }
      } catch (err) {
    ...
    if (categoryId && productId) fetchProducts();
    else setLoading(false);
  }, [categoryId, productId, slug]);
  ...
  const handleShare = () => {
    const shareUrl =
      p.shareLink ||
      `https://paltan.com/product/${p.slug || p._id}?productId=${p._id}`; // 🔹 slug based
