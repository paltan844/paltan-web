...
const ProductItem: FC<{
  ...
  const { getItemCount } = useCartStore();
  const count = getItemCount(item._id); // optional if needed

  const handleNavigation = () => {

    // ✅ Safe route detection (works for dashboard + search + category + cursor)
    const route =
      item?.CategoryById?.navigateTo ||
      item?.navigateTo ||
      item?.category?.navigateTo ||
      null;
    ...
    <View style={{ flex: 1 }}>
      ...
      >
        ...
        <View style={styles.content}>
          ...
          >
            <CustomText>
              Buy {item.name} online in {CITY} at best price from Paltan Shopping Mall
            </CustomText>
          </View>
