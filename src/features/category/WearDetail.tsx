...
import KeyFeaturesModalWear from './KeyFeatureModalWear';
  ...
  const fetchProduct = async () => {
    ...
    if (true) {
      setExtraDetails({
        ...
        size: data?.sellerAddress,
         warranty: data?.warranty,
        color: data?.shelfLife,
        fabricCare: data?.type ,
        returnPolicy: data?.returnPolicy ,
        countryOfOrigin: data?.countryOfOrigin,
        suitableFor: data?.seller,
...
<View style={styles.featuresRow}>
  ...
  {p.quantity && p.quantity !== 'N/A' && (
    <>
      <CustomText variant="h7">Quantity</CustomText>
      <CustomText variant="h8" fontFamily={Fonts.SemiBold} style={{ opacity: 0.7 }}>{p.quantity}</CustomText>
    </>
  )}

  {product.dietType && product.dietType !== 'N/A'  && (
    <>
      <CustomText variant="h7" style={{ marginTop: 10 }}>Size</CustomText>
...
export default WithCart(WearDetail);   */
